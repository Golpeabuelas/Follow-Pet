    ...
    return (
        ...
        >
            ...
            {notifications.length === 0 ? (
                <Text className="text-[#777] text-base text-center">No tienes notificaciones.</Text>
            ) : (
                notifications.map((noti) => (
                    <Pressable key={noti.id_notificacion} onPress={() => handleMarkAsRead(noti.id_notificacion)} className="relative mb-4">
                        ...
                        {noti.estado.toLowerCase() === "no leída" && (
                            <View className="absolute inset-0 bg-blue-300 opacity-25 rounded-xl" pointerEvents="none" />
                        )}
                    </Pressable>
                ))
            )}
